...
public:
  ...
  public:
    HashTableIterator(ListOfValues **P, size_t Pos, size_t ListPos, size_t Size)
        : p(P), pos(Pos), list_pos(ListPos), size(Size){};
    HashTableIterator(const HashTableIterator &it)
        : p(it.p), pos(it.pos), list_pos(it.list_pos), size(it.size) {}
    ...
    bool operator==(HashTableIterator const &other) const {
      return p == other.p and pos == other.pos and
  ...
  iterator end() {
    for (size_t i = capacity - 1; i >= 0; --i) {
      if (H[i]) {
        ListOfValues *elem = H[i];
        size_t pos = 0;
        while (elem) {
          elem = getNext(*elem);
          ++pos;
        }
        return iterator(H, pos, i, capacity);
      }
    }
    return iterator(H, 0, capacity, capacity);
  }
  ...
  bool insert(const Key &k, const Value &v) {
    ...
    auto *ptr = H[key]; // insert an element at the end of the list, if elements
                        // with such a key exist
...
private:
  ...
  void resize() {
    ...
    std::swap(H, _H);
    for (int i = 0; i < old_tsize;
         i++) { // add all elements of the old array to the new one
  ...
  static const Key &getKey(const ListOfValues &v) {
    return v.key;
  } // the "key" field must be unavailable for editing
